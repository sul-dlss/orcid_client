...
RSpec.describe SulOrcidClient::WorkMapper do
  describe ".map" do
    context "when full record" do
      ...
      let(:work) do
        described_class.map(description: description, doi: "10.25740/hj302gv2126")
      end
      ...
      it "is accepted by orcid api" do
        ...
        VCR.use_cassette("Work_Mapper/_map/accepted_by_orcid_api") do
          expect(client.add_work(orcidid: "https://sandbox.orcid.org/0000-0003-3437-349X", work: work, token: "FAKE-294e-4bc8-8afd-96315b06ae04")).to eq("1755750")
        end
      end
    end
    ...
    context "when minimal record" do
      ...
      it "maps" do
        expect(described_class.map(description: description)).to eq(
          title: {
            title: {
              value: "Strategies for Digital Library Migration"
            }
    ...
    context "when no title" do
      ...
      it "raises" do
        expect { described_class.map(description: description) }.to raise_error(SulOrcidClient::WorkMapper::WorkMapperError)
      end
    end
  end
  ...
  describe ".parse_date" do
    ...
    it "parses" do
      expect(mapper.send(:parse_date, "2021-08-31")).to eq(["2021", "08", "31"])
